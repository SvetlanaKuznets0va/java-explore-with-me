...
public interface StatsRepository extends JpaRepository<StatsModel, Integer> {
    ...
    @Query("select new ru.practicum.dto.StatsDto(s.app, s.uri, count(distinct s.ip)) from StatsModel as s " +
    ...
    @Query("select new ru.practicum.dto.StatsDto(s.app, s.uri, count(s.ip)) from StatsModel as s " +
            "where s.timestamp between ?1 and ?2 " +
